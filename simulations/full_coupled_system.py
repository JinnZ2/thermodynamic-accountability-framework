...
def health(n):
    ...
    for var, floor in CRITICAL.items():
        val = d[var]
        scores[var] = max(0,(floor-val)/floor) if var in INVERTED else max(0,(val-floor)/max(1-floor, 1e-9))
...
class Lag:
    def __init__(self,d,v): self.buf=deque([v]*d,maxlen=d)
...
def run(stress, steps=500, onset=60):
    ...
    for t in range(steps):
        ...
        for k in n.__dict__: hist[k].append(getattr(n,k))
        h,lim = health(n)
