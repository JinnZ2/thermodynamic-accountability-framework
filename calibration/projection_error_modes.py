...
FalsifiableClaim(
    ...
    known_evidence=(
        ...
        "Haílzaqv (Heiltsuk) bear ecology: place-based knowledge guided "
...
FalsifiableClaim(
    ...
    known_evidence=(
        ...
        "ΔM_earth = M_in − M_out is a closed equation. The financial "
...
# (event → sensor → report → analyst → summary
# → dashboard → meeting → exec → decision)
...
def thermodynamic_alignment(self) -> float:
    ...
    # Top tier: what touches physics directly
    # Mid tier: what enables physics-grounded operation
    # Base tier: what recognizes the full substrate
    weights = [
...
def verdict(self) -> str:
    ...
    if ta >= 0.7:  return "PHYSICS-GROUNDED"
    if ta >= 0.4:  return "MIXED — partial faith-based operation"
...
signal_fidelity=0.50,       # workers closer to signal but still meeting-gated
...
money_physics_coupling=0.40, # tracks money but also tracks parts, hours, returns
),
SystemScore(
...
```
print_divider("SUBSTRATE AUDIT — Thermodynamic Analysis of Hierarchical Capital Control")
...
for c in CLAIMS:
    icon = {"PASS": "✓", "FAIL": "✗", "UNTESTED": "?", "CIRCULAR": "⟳"}
    print(f"\n  [{icon[c.verdict.value]}] {c.id}: {c.claim[:80]}...")
    print(f"      H₀: {c.null_hypothesis[:75]}...")
...
print(f"  Diagnosis: {'Self-reinforcing hierarchy, maintainers excluded' if closed and excluded else 'Check topology'}")
print()
for n in CAUSAL_LOOP:
    arrow = " → " + ", ".join(n.drives) if n.drives else " → [EXCLUDED]"
    flag = " ⟲" if n.is_self_reinforcing else ""
    print(f"  {n.id}{arrow}{flag}")

# DMAIC
print_divider("SIX SIGMA DMAIC AUDIT")
for d in DMAIC_AUDIT:
    icon = {"PASS": "✓", "FAIL": "✗", "CIRCULAR": "⟳"}[d.verdict.value]
    print(f"\n  [{icon}] {d.phase}")
...
EVIDENCE:
- 0/{n} claims falsified by the hierarchy’s own evidence
- {p}/{n} claims PASS on independent evidence
- {u}/{n} claims UNTESTED (experiments never run)
- {c}/{n} claims CIRCULAR (self-referencing justification)”””.format(
n=len(CLAIMS),
p=sum(1 for c in CLAIMS if c.verdict == Verdict.PASS),
u=sum(1 for c in CLAIMS if c.verdict == Verdict.UNTESTED),
c=sum(1 for c in CLAIMS if c.verdict == Verdict.CIRCULAR),
) + “””
