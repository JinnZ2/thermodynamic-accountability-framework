...
Example: “flow”
physics definition:  m³/s with kinematic viscosity
...
become interchangeable because the labeler pool doesn’t
...
# If the document is about the locked substrate AND uses drift markers,

# it’s in metaphor-drift territory.
...
“m^3”, “m³”, “cubic meter”, “volumetric”, “kinematic”,
...
“valid_markers”: [
“kg*m/s”, “kg·m/s”, “mass times velocity”,
...
“ is increasing” + “ “ + “(without substrate)”,
“political entropy”,
],
“valid_markers”: [
...
def audit(self, text: str) -> AtrophyReport:
    ...
    for term, spec in self.locked.items():
        ...
        if drift_hits > 0 and not grounded:
            ...
            status = "YELLOW"
            notes = (f"Term present but unanchored. "
                     f"No drift markers, no substrate anchors.")
...
1. Does it appear with drift markers (social metaphor use)?
1. Classify: GREEN (anchored), YELLOW (mixed), RED (pure metaphor).

AXES

1. drift_ratio       average drift-to-use ratio
   (green <= 0.20, yellow <= 0.50) INVERTED
1. drift_term_count  fraction of terms showing drift
   (green <= 0.20, yellow <= 0.40) INVERTED
1. grounding_ratio   fraction of terms with substrate anchors
   (green >= 0.70, yellow >= 0.40)
1. unit_lock_count   total substrate anchors present
   (green >= 3, yellow >= 1)
