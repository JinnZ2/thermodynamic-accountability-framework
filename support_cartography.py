...
Mirrored from upstream:
    github.com/JinnZ2/Resilience-indigenous-worldwide/
    tree/main/resilience_stack/support_cartography.py
"""
...
class GateClassifier:
    ...
    def classify(self, cc: CognitionClass) -> list[GateFailure]:
        ...
        if cc.emission_profile and any(
            ...
            for marker in ["rarely narrated", "actively avoids",
                           "internal", "leaves no invoice"]
        ):
            failures.append(GateFailure(
                gate=Gate.EMISSION,
                reason=f"Does not externalize in instrumentable form: "
                       f"{cc.emission_profile}",
                recovery_requires=[
        ...
        if (cc.requires_continuous_signal
                or cc.requires_full_sensorimotor
                or cc.requires_long_horizon):
            reasons = []
            if cc.requires_continuous_signal:
                reasons.append("requires continuous signal "
                               "(discrete sensors inadequate)")
            if cc.requires_full_sensorimotor:
                reasons.append("requires full sensorimotor loop "
                               "(text/image projections insufficient)")
            if cc.requires_long_horizon:
                reasons.append("requires long-horizon trajectories "
                               "(snapshots lose the structure)")
            failures.append(GateFailure(
                gate=Gate.CAPTURE,
                reason="; ".join(reasons),
                recovery_requires=[
        ...
        if cc.economic_incentive_to_document < 0.2 or not cc.legible_to_classifier:
            ...
            if cc.economic_incentive_to_document < 0.2:
                reasons.append(f"low incentive to document "
                               f"(score={cc.economic_incentive_to_document})")
...
class SupportBoundaryMapper:
    ...
    def map_region(self, cc: CognitionClass) -> Optional[ZeroSupportRegion]:
        ...
        proxies = [
            ...
            "correlation between AI forecast error and density of "
            "non-emitting knowledge holders",
        ]
...
class CollapseRateEstimator:
    ...
    def estimate(
        ...
        if pN < 1 and alpha < 0.2:
            regime = "collapse"
            notes.append("pN<1 AND low real-data fraction: rapid mode "
                         "extinction expected.")
        elif pN < 1:
            regime = "metastable"
            ...
            regime = "metastable"
            notes.append("Low real-data fraction: synthetic recursion will "
                         "erode tails.")
        else:
            regime = "stable"
            notes.append("Adequate sampling and real-data fraction; core "
                         "distribution preserved.")

        if half_life != float("inf") and half_life < 5:
            notes.append(f"Half-life ~{half_life:.2f} iterations. "
                         "Mode likely gone fast.")
...
class ProjectionLossResult:
    ...
    modes_collapsed_to_one: list[list[int]]  # groups that became indistinguishable
...
class ProjectionLossSimulator:
    ...
    Even if a mode is PRESENT in reality, projection onto the observable
    basis can collapse distinct modes into one. This is different from
    sampling loss. No amount of data fixes it -- you need a richer
    observation basis.
    ...
    def simulate(
        ...
        observation_basis: list[list[float]],  # rows are projection axes
        ...
        # Project each mode through the observation basis
        projected = []
        for mode in true_modes:
            proj = [sum(row[i] * mode[i] for i in range(true_dim))
                    for row in observation_basis]
            projected.append(proj)

        # Group modes that project to nearly-identical points
        n = len(true_modes)
        ...
        if collapsed:
            ...
            notes.append("This loss is NOT recoverable by more samples. "
                         "Requires richer observation basis.")
        else:
            notes.append("All true modes remain distinguishable under "
                         "this projection.")

        if obs_dim < true_dim:
            notes.append(
                f"Observation basis ({obs_dim}D) has lower rank than "
                f"true space ({true_dim}D); projection loss is "
                f"structurally guaranteed for some configurations."
            )
...
class SupportCartography:
    ...
    def full_report(
        self,
        rare_mode_tests: list[tuple[float, int, float]],  # (p, N, alpha)
        ...
        if projection_test:
            proj_result = asdict(
                self.projection_simulator.simulate(**projection_test))

        summary = {
            "zero_support_region_count": len(regions),
            "cognition_classes_audited": len(self.cognition_registry),
            "structural_exclusion_rate": (len(regions)
                                          / len(self.cognition_registry)),
...
if __name__ == "__main__":
    ...
    # blind to z. Modes separated only in z collapse.
    projection_test = {
        "true_modes": [
